...
module android.ndk.hardware_buffer;
...
struct AHardwareBuffer_Desc
{
     ...
     */
    uint layers;
    uint format; ///< One of AHardwareBuffer_Format.
    ulong usage; ///< Combination of AHardwareBuffer_UsageFlags.
...
struct AHardwareBuffer_Plane
{
    ...
    uint pixelStride; ///< Distance in bytes from the color channel of one pixel to the next
    uint rowStride; ///< Distance in bytes from the first value of one row of the image to
...
{
    uint planeCount; ///< Number of distinct planes
 ...
 *
 * \return 0 on success, -EINVAL if \a buffer is NUL, or an error
 ...
 *
 * \return 0 on success, -EINVAL if \a outBuffer is NUL, or an error
 * number if the operation fails for any reason.
 */
